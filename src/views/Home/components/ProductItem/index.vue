<template>
  <div class="x-product-item">
    ...
    <div
      ...
      :key="item.cardData.source"
      @click="$emit('onClick', item.cardData.execution_plan.goodId)"
    >
      <!-- S 商品图片区域 -->
      <div class="x-product-img">
        <span class="x-subscript">{{ item.sortNo }}</span>
        <img :src="filterProductImg(item.cardData?.image_info[0]?.web_url)" alt="商品图片">
      </div>
      <!-- E 商品图片区域 -->

      <!-- S 商品信息区域 -->
      <div class="x-product-info flex flex-col">
        <h3>{{ item.cardData.source }}</h3>
        <!-- S 商品标签和销售 -->
        <div class="flex items-center justify-between">
          ...
          <span class="x-sale-count">已售 {{ item.cardData.sold_count }}</span>
        </div>
        <!-- E 商品标签和销售 -->
        ...
        <div class="x-price-box flex justify-between item-center">
          <label class="x-real-price">
            <span class="x-old-price">{{ item.cardData.origin_amount }}</span>
            <span class="x-price">{{ item.cardData.actual_amount }}</span>
          </label>
          <label class="x-discount-price">
            <span>返</span>
            <span class="x-price">￥{{ item.cardData.actual_amount_num }}</span>
          </label>
        </div>
         <!-- E 商品价格和优惠区域 -->
      </div>
